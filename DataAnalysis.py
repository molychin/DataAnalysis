...
In today's business, data analysis is playing a role in making decisions more scientific 
and helping the business achieve effective operation.
数据分析是一个检查、清理、转换和建模数据的过程，目的是发现有用的信息、得出结论和支持决策。数据分析
有多个方面和方法，包括各种名称下的各种技术，同时用于不同的商业、科学和社会科学领域。
在当今的企业中，数据分析在使决策更加科学和帮助企业实现有效运营方面发挥着作用。
...
数据挖掘是一种特殊的数据分析技术，它专注于建模和知识发现，以实现预测而非纯粹的描述性目的，而商务智
能则涵盖了严重依赖于聚合的数据分析
...
数据是从各种来源的文集。分析师可以将需求传达给数据保管人，例如组织内的信息技术人员。还可以从环境中的
传感器（如交通摄像机、卫星、记录设备等）收集数据。还可以通过访谈、在线资源下载或阅读文档获取数据。


●Data processing   数据处理

●Data cleaning    数据清洗
一旦处理和组织，数据可能不完整、包含重复或包含错误。由于数据输入和存储方式存在问题，因此需要进行数据清理。
数据清理是防止和纠正这些错误的过程。常见的任务包括记录匹配、识别数据的不准确、现有数据的总体质量、重复数
据消除和列分段。此类数据问题也可以通过各种分析技术来识别。例如，根据财务信息，可将特定变量的总数与独立公
布的可靠数字进行比较。也可审查高于或低于预先确定阈值的异常金额。有几种类型的数据清理取决于数据类型，如
电话号码、电子邮件地址、雇主等。可以使用离群值检测的定量数据方法来消除可能输入错误的数据。文本数据拼写
检查可以用来减少输入错误的单词数量，但是很难判断单词本身是否正确。
...
●错误3：观测维度有误，原因：刻意挑选的数据维度；两个航空公司航班延误率的数据分析；这种忽略某个数据维度，
导致统计结论完全相反的现象，称为“辛普森悖论”
...
■使用统计图表的三个原则
多种图表辅助观点的表达，可使统计报告更加清晰、易读。但如果运用得不好，图表不但不能支撑分析结论，
还会对表达的观点造成干扰。
原则1：目标原则
很多统计图表的表达效果不好，均是因为使用者头脑中没有清晰的目标。为什么要制作这张图？它表达的内容是
不是最贴切的？表达的形式是不是最合理的？有人认为“好的图表就是形式上的美观炫丽”，这是根本性的错误。
统计图表的第一要务是最大程度地辅助观点表达，而美观只是末节。
原则2：极简原则
不仅图表需要极简原则，撰写分析报告也是一样。无关的信息越多，想表达的东西越淡。
原则3：不扭曲原则
虽然图表是为了辅助表达观点而存在的，但也不能为了表达观点而故意扭曲图形（例如选择不恰当的坐标值和比例）

当影响一个事物结果的各种因素随机发生，根据中心极限定理，它们的总的平均表现就是正态分布。
...
2.收集整理信息:搜集、整理关于要解决问题的历史资料、类似情况和现状。 例如，从现有的报表数据中就能够
看到当前问题点的数据情况或者一段时间的趋势；
...
根据分析的结果，得出一些当前问题产生的一些结论。这里注意分析的方法以及维度，结果的展示方式等。 结论需要求足够的数据做支撑。
...
将思想和观点形象化的表达，加深读者或听众的印象。 使用图表时，必须明确通过图表要表达的信息是什么。
...
针对数据分析结论，给出当前问题的解决建议措施。 一方面从业务层面进行建议措施，另一方面，可以就问题点进行更深层次分析，给出数据挖掘层面的解决措施。


7.实施效果评估及报告整理 


根据措施实施效果进行实际评估，将完成的分析过程，结果以及评估整理报告，为以后出现问题提供经 验教训。 对于本次没有完全解决的问题，进行说明。
...
数据挖掘时往往数据量非常大，在大量数据上进行挖掘分析需要很长的时间，数据归约技术可以用来得到数据集的归约表示，它小得多，但仍然接近于保持原数据的完整性，并结果与归约前结果相同或几乎相同。
...
逻辑检查主要是审核数据是否符合逻辑，内容是否合理，各项目或数字之间有无相互矛盾的现象，此方法主要适合对定性（品质）数据的审核。
...
计算检查是检查调查表中的各项数据在计算结果和计算方法上有无错误，主要用于对定量（数值型）数据的审核。
...
对于使用者来说，首先应该弄清楚数据的来源、数据的口径以及有关的背景资料，以便确定这些资料是否符合自己分析研究的需要，是否需要重新加工整理等，不能盲目生搬硬套。此外，还要对数据的时效性进行审核，对于有些时效性较强的问题，如果取得的数据过于滞后，可能失去了研究的意义。一般来说，应尽可能使用最新的统计数据。
...
数据筛选包括两方面的内容：一是将某些不符合要求的数据或有明显错误地数据予以剔除；二是将符合某种特定条件的数据筛选出来，对不符合特定条件的数据予以剔除。数据的筛选在市场调查、经济分析、管理决策中是十分重要的。
...
Data analysis is a process of inspecting, cleansing, transforming, and modeling data with the goal of discovering useful information, informing conclusions, and supporting decision-making. 数据分析是一个检查、清理、转换和建模数据的过程，目的是发现有用的信息、得出结论和支持决策。
...
阅读数据之后，要站在更高的角度去看这些数据，站在更高的位置上，从更长远的观点来看
...
数据是一个过程的结果反映，怎样通过看数据找到更多的原因隐藏在现象背后的真相，需要我们下切思维，把事物切细了分析，把过程拆分细了分析。 知道数据的构成、分解数据的手段、对分解后的数据的重要程度的了解。
...
找到共性的东西进行分析，要客观。 现在的整体数据表现出什么问题？是否有规律可行？
